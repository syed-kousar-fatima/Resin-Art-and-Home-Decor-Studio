...
/* =========================================================
   workshops.js — ResinLux Studio
   Theme · Scroll Reveal · Carousel · Booking Form
   ========================================================= */
...
(function () {
  ...
  function initForm() {
    ...
    form.addEventListener('submit', e => {
      ...
      const guests   = document.getElementById('bf-guests');
      const workshop = document.getElementById('bf-workshop');
      let valid = true;

      [name, email, guests, workshop].forEach(el => {
        const empty = !el.value.trim();
        const badEmail = el.type === 'email' && !el.value.includes('@');
        if (empty || badEmail) {
