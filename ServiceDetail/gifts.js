...
(function () {
  ...
  function initForm() {
    ...
    form.addEventListener('submit', e => {
      ...
      if (!name.value.trim()) {
        setError('gf-name', 'gf-name-err', 'Please enter your name.');
        valid = false;
      } else { setError('gf-name', 'gf-name-err', ''); }

      /* Email */
      const email = document.getElementById('gf-email');
      if (!email.value.trim()) {
        setError('gf-email', 'gf-email-err', 'Please enter your email address.');
        valid = false;
      } else if (!emailRx.test(email.value)) {
        setError('gf-email', 'gf-email-err', 'Please enter a valid email address.');
        valid = false;
      } else { setError('gf-email', 'gf-email-err', ''); }

      /* Occasion */
      const occasion = document.getElementById('gf-occasion');
      if (!occasion.value) {
        setError('gf-occasion', 'gf-occasion-err', 'Please select an occasion.');
        valid = false;
      } else { setError('gf-occasion', 'gf-occasion-err', ''); }

      /* Recipient name */
      const recipient = document.getElementById('gf-recipient');
      if (!recipient.value.trim()) {
        setError('gf-recipient', 'gf-recipient-err', 'Please enter the recipient’s name for personalisation.');
        valid = false;
      } else { setError('gf-recipient', 'gf-recipient-err', ''); }

      /* Deadline date */
      const deadline = document.getElementById('gf-deadline');
      if (!deadline.value) {
        setError('gf-deadline', 'gf-deadline-err', 'Please select a required-by date.');
        valid = false;
      } else {
        ...
        if (selected < today) {
          setError('gf-deadline', 'gf-deadline-err', 'Please select a future date.');
          valid = false;
        } else { setError('gf-deadline', 'gf-deadline-err', ''); }
      }

      /* Details */
      const details = document.getElementById('gf-details');
      if (!details.value.trim() || details.value.trim().length < 15) {
        setError('gf-details', 'gf-details-err', 'Please describe your colour and personalisation preferences (at least 15 characters).');
        valid = false;
      } else { setError('gf-details', 'gf-details-err', ''); }

      /* Agree */
      const agree    = document.getElementById('gf-agree');
      const agreeErr = document.getElementById('gf-agree-err');
      if (!agree.checked) {
