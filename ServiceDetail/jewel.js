...
(function () {
  ...
  /* ── FORM VALIDATION ── */
  function initForm() {
    const form    = document.getElementById('jewForm');
    const success = document.getElementById('jewSuccess');
    if (!form) return;
    ...
    form.addEventListener('submit', e => {
      ...
      if (!name.value.trim()) {
        setError('jf-name', 'jf-name-err', 'Please enter your full name.');
        valid = false;
      } else { setError('jf-name', 'jf-name-err', ''); }

      /* Email */
      const email = document.getElementById('jf-email');
      if (!email.value.trim()) {
        setError('jf-email', 'jf-email-err', 'Please enter your email address.');
        valid = false;
      } else if (!emailRx.test(email.value)) {
        setError('jf-email', 'jf-email-err', 'Please enter a valid email address.');
        valid = false;
      } else { setError('jf-email', 'jf-email-err', ''); }

      /* Type */
      const type = document.getElementById('jf-type');
      if (!type.value) {
        setError('jf-type', 'jf-type-err', 'Please select a jewellery type.');
        valid = false;
      } else { setError('jf-type', 'jf-type-err', ''); }

      /* Metal */
      const metal = document.getElementById('jf-metal');
      if (!metal.value) {
        setError('jf-metal', 'jf-metal-err', 'Please select a metal preference.');
        valid = false;
      } else { setError('jf-metal', 'jf-metal-err', ''); }

      /* Colours */
      const colours = document.getElementById('jf-colours');
      if (!colours.value.trim() || colours.value.trim().length < 15) {
        setError('jf-colours', 'jf-colours-err', 'Please describe your colour and inclusion preferences (at least 15 characters).');
        valid = false;
      } else { setError('jf-colours', 'jf-colours-err', ''); }

      /* Agree */
      const agree    = document.getElementById('jf-agree');
      const agreeErr = document.getElementById('jf-agree-err');
      if (!agree.checked) {
