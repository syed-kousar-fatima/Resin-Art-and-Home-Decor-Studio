...
(function () {
  ...
  function initForm() {
    ...
    const success = document.getElementById('corpSuccess');
    if (!form) return;

    /* Helper: set / clear an error */
    ...
    form.addEventListener('submit', e => {
      ...
      if (!name.value.trim()) {
        valid = setError('cf-name', 'cf-name-err', 'Please enter your full name.');
      } else {
        setError('cf-name', 'cf-name-err', '');
      }
      ...
      const emailRx = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!email.value.trim()) {
        valid = setError('cf-email', 'cf-email-err', 'Please enter your work email.');
      } else if (!emailRx.test(email.value)) {
        valid = setError('cf-email', 'cf-email-err', 'Please enter a valid email address.');
      } else {
        setError('cf-email', 'cf-email-err', '');
      }

      /* Company */
      const company = document.getElementById('cf-company');
      if (!company.value.trim()) {
        valid = setError('cf-company', 'cf-company-err', 'Please enter your company name.');
      } else {
        setError('cf-company', 'cf-company-err', '');
      }

      /* Quantity */
      const qty = document.getElementById('cf-qty');
      if (!qty.value) {
        valid = setError('cf-qty', 'cf-qty-err', 'Please select an estimated quantity.');
      } else {
        setError('cf-qty', 'cf-qty-err', '');
      }

      /* Product */
      const product = document.getElementById('cf-product');
      if (!product.value) {
        valid = setError('cf-product', 'cf-product-err', 'Please select a gift type.');
      } else {
        setError('cf-product', 'cf-product-err', '');
      }

      /* Brief */
      const brief = document.getElementById('cf-brief');
      if (!brief.value.trim() || brief.value.trim().length < 20) {
        valid = setError('cf-brief', 'cf-brief-err', 'Please provide a brief description (at least 20 characters).');
      } else {
        setError('cf-brief', 'cf-brief-err', '');
      }

      /* Agree checkbox */
      const agree    = document.getElementById('cf-agree');
      const agreeErr = document.getElementById('cf-agree-err');
      if (!agree.checked) {
        if (agreeErr) agreeErr.textContent = 'You must agree to be contacted to submit this form.';
        valid = false;
      } else {
