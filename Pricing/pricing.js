...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initCarousel(trackId, prevId, nextId, dotsId) {
    ...
    function goTo(idx) {
      ...
      track.style.transform = 'translateX(-' + (current * slides[0].getBoundingClientRect().width) + 'px)';
      updateDots();
    }
  ...
  initCarousel('featTrack', 'featPrev', 'featNext', 'featDots');

  /* ── FAQ Accordion ── */
  var faqAccordion = document.getElementById('faqAccordion');
  if (faqAccordion) {
    faqAccordion.querySelectorAll('.faq-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var isOpen = btn.getAttribute('aria-expanded') === 'true';
        faqAccordion.querySelectorAll('.faq-btn').forEach(function (b) {
          b.setAttribute('aria-expanded', 'false');
        });
        faqAccordion.querySelectorAll('.faq-body').forEach(function (body) {
  ...
  var fields = {
    name:  { el: document.getElementById('e-name'),  err: document.getElementById('err-name') },
    email: { el: document.getElementById('e-email'), err: document.getElementById('err-email') },
    plan:  { el: document.getElementById('e-plan'),  err: document.getElementById('err-plan') },
    level: { el: document.getElementById('e-level'), err: document.getElementById('err-level') },
    goals: { el: document.getElementById('e-goals'), err: document.getElementById('err-goals') },
    agree: { el: document.getElementById('e-agree'), err: document.getElementById('err-agree') }
  };

  function clearErr(f) { if (f.el) f.el.classList.remove('error'); if (f.err) f.err.textContent = ''; }
  function showErr(f, msg) { if (f.el) f.el.classList.add('error'); if (f.err) f.err.textContent = msg; }
  ...
  form.addEventListener('submit', function (e) {
    ...
    /* Email */
    var ev = fields.email.el.value.trim();
    var er = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!ev) {
    ...
    if (!fields.plan.el.value) {
      showErr(fields.plan, 'Please select a plan you are interested in.'); ok = false;
    } else { clearErr(fields.plan); }

    /* Level */
    if (!fields.level.el.value) {
      showErr(fields.level, 'Please select your experience level.'); ok = false;
    } else { clearErr(fields.level); }

    /* Goals */
    var gv = fields.goals.el.value.trim();
    if (!gv) {
      showErr(fields.goals, 'Please briefly describe your goals.'); ok = false;
    } else if (gv.length < 15) {
      showErr(fields.goals, 'Please provide a bit more detail (at least 15 characters).'); ok = false;
    } else { clearErr(fields.goals); }

    /* Agree */
    if (!fields.agree.el.checked) {
      showErr(fields.agree, 'Please agree to being contacted.'); ok = false;
