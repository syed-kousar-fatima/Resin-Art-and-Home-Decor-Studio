...
/* =======================================
   COLOUR INSPIRATION — Blog Detail JS
   ======================================= */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  initCarousel('paletteTrack', 'palettePrev', 'paletteNext', 'paletteDots');

  /* ── Colour Quiz Form Validation ── */
  var form        = document.getElementById('colourQuizForm');
  var formSuccess = document.getElementById('formSuccess');
  if (!form) return;

  var fields = {
    name:  { el: document.getElementById('q-name'),  err: document.getElementById('err-name') },
    email: { el: document.getElementById('q-email'), err: document.getElementById('err-email') },
    style: { el: document.getElementById('q-style'), err: document.getElementById('err-style') },
    use:   { el: document.getElementById('q-use'),   err: document.getElementById('err-use') },
    agree: { el: document.getElementById('q-agree'), err: document.getElementById('err-agree') }
  };

  function clearErr(f) { f.el.classList.remove('error'); f.err.textContent = ''; }
  function showErr(f, msg) { f.el.classList.add('error'); f.err.textContent = msg; }
  ...
  form.addEventListener('submit', function (e) {
    ...
    /* Style */
    if (!fields.style.el.value) {
      showErr(fields.style, 'Please select your preferred resin style.'); ok = false;
    } else { clearErr(fields.style); }

    /* Use */
    if (!fields.use.el.value) {
      showErr(fields.use, 'Please select your intended use.'); ok = false;
    } else { clearErr(fields.use); }

    /* Mood (radio) */
    var moodErr   = document.getElementById('err-mood');
    var moodChecked = form.querySelector('input[name="mood"]:checked');
    if (!moodChecked) {
      moodErr.textContent = 'Please select a mood.'; ok = false;
    } else { moodErr.textContent = ''; }

    /* Agree */
    if (!fields.agree.el.checked) {
      showErr(fields.agree, 'You must agree to receive emails to subscribe.'); ok = false;
    } else { clearErr(fields.agree); }

    if (!ok) {
      var firstErr = form.querySelector('.error, .form-error:not(:empty)');
