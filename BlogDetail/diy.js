...
document.addEventListener('DOMContentLoaded', function () {
  ...
  initCarousel('projectTrack', 'projectPrev', 'projectNext', 'projectDots');

  /* ── DIY Help Form Validation ── */
  var form        = document.getElementById('diyHelpForm');
  var formSuccess = document.getElementById('formSuccess');
  if (!form) return;

  var fields = {
    name:     { el: document.getElementById('d-name'),     err: document.getElementById('err-name') },
    email:    { el: document.getElementById('d-email'),    err: document.getElementById('err-email') },
    level:    { el: document.getElementById('d-level'),    err: document.getElementById('err-level') },
    project:  { el: document.getElementById('d-project'),  err: document.getElementById('err-project') },
    question: { el: document.getElementById('d-question'), err: document.getElementById('err-question') },
    agree:    { el: document.getElementById('d-agree'),    err: document.getElementById('err-agree') }
  };
  ...
  form.addEventListener('submit', function (e) {
    ...
    /* Project */
    if (!fields.project.el.value) {
      showErr(fields.project, 'Please select the project type.'); ok = false;
    } else { clearErr(fields.project); }

    /* Question */
    var qv = fields.question.el.value.trim();
    if (!qv) {
      showErr(fields.question, 'Please describe your question or issue.'); ok = false;
    } else if (qv.length < 20) {
      showErr(fields.question, 'Please provide a bit more detail (at least 20 characters).'); ok = false;
    } else { clearErr(fields.question); }

    /* Agree */
    if (!fields.agree.el.checked) {
      showErr(fields.agree, 'Please agree to receive a reply.'); ok = false;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    navLinks.forEach(link => {
        ...
        if (
            href === currentPage ||
            (
                (currentPage === "home.html" || currentPage === "blog.html") &&
                href === "blog.html"
            )
        ) {
